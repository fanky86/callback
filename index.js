...
// ✅ Tambahan untuk menangani POST /login (agar tidak error)
app.post('/login', (req, res) => {
    const { email, password } = req.body;
    // Ini hanya simulasi, tidak digunakan untuk login asli
    res.send(`
        <h2>Login Manual</h2>
        <p>Email: ${email}</p>
        <p>Password: ${password}</p>
        <p><i>Catatan: Ini hanya untuk contoh. Tidak ada autentikasi dilakukan.</i></p>
    `);
});
